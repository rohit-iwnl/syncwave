...
class RoomCaptureUIView: UIView, RoomCaptureSessionDelegate {
    ...
    func captureSession(
        _ session: RoomCaptureSession, didEndWith data: CapturedRoomData,
        error: Error?
    ) {
        if let error = error {
    ...
    func processScannedData() async {
        ...
        do {
            ...
            guard
                let catalogBundle = Bundle.main.url(
            ...
            try processedRoom.export(
                to: destinationURL, modelProvider: modelProvider,
                exportOptions: .model)
